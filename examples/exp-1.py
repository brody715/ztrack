...
def main():
    with ztrack.create(result_dir=f"{ztrack.str_cwd()}/tracks-{ztrack.str_datetime()}") as tracker:
        ...
        tracker.track({
            'name': 'hello, world',
            'image-2': ztrack.Artifact(data, format='png', save_func=lambda data, path: cv2.imwrite(path)),
        }, commit=False)

        tracker.track({
            'models': ztrack.Artifact(data, save_fn=lambda data, path: torch.save(data, path))
        })
        ...
        tracker.with_fields({
            'name': 'asdf',
            'image_1': ztrack.Image()
        }).info('hello, world', exclude_fields=['image-1']).track()
